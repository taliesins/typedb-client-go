//
// Copyright (C) 2022 Vaticle
//
// This program is free software: you can redistribute it and/or modify
// it under the terms of the GNU Affero General Public License as
// published by the Free Software Foundation, either version 3 of the
// License, or (at your option) any later version.
//
// This program is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU Affero General Public License for more details.
//
// You should have received a copy of the GNU Affero General Public License
// along with this program.  If not, see <https://www.gnu.org/licenses/>.
//

// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.30.0
// 	protoc        v4.22.2
// source: v2/protobuf/core/core_service.proto

package typedb_protocol

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

var File_v2_protobuf_core_core_service_proto protoreflect.FileDescriptor

var file_v2_protobuf_core_core_service_proto_rawDesc = []byte{
	,
}

var file_v2_protobuf_core_core_service_proto_goTypes = []interface{}{
	(*CoreDatabaseManager_Contains_Req)(nil), // 0: typedb.protocol.CoreDatabaseManager.Contains.Req
	(*CoreDatabaseManager_Create_Req)(nil),   // 1: typedb.protocol.CoreDatabaseManager.Create.Req
	(*CoreDatabaseManager_All_Req)(nil),      // 2: typedb.protocol.CoreDatabaseManager.All.Req
	(*CoreDatabase_Schema_Req)(nil),          // 3: typedb.protocol.CoreDatabase.Schema.Req
	(*CoreDatabase_TypeSchema_Req)(nil),      // 4: typedb.protocol.CoreDatabase.TypeSchema.Req
	(*CoreDatabase_RuleSchema_Req)(nil),      // 5: typedb.protocol.CoreDatabase.RuleSchema.Req
	(*CoreDatabase_Delete_Req)(nil),          // 6: typedb.protocol.CoreDatabase.Delete.Req
	(*Session_Open_Req)(nil),                 // 7: typedb.protocol.Session.Open.Req
	(*Session_Close_Req)(nil),                // 8: typedb.protocol.Session.Close.Req
	(*Session_Pulse_Req)(nil),                // 9: typedb.protocol.Session.Pulse.Req
	(*Transaction_Client)(nil),               // 10: typedb.protocol.Transaction.Client
	(*CoreDatabaseManager_Contains_Res)(nil), // 11: typedb.protocol.CoreDatabaseManager.Contains.Res
	(*CoreDatabaseManager_Create_Res)(nil),   // 12: typedb.protocol.CoreDatabaseManager.Create.Res
	(*CoreDatabaseManager_All_Res)(nil),      // 13: typedb.protocol.CoreDatabaseManager.All.Res
	(*CoreDatabase_Schema_Res)(nil),          // 14: typedb.protocol.CoreDatabase.Schema.Res
	(*CoreDatabase_TypeSchema_Res)(nil),      // 15: typedb.protocol.CoreDatabase.TypeSchema.Res
	(*CoreDatabase_RuleSchema_Res)(nil),      // 16: typedb.protocol.CoreDatabase.RuleSchema.Res
	(*CoreDatabase_Delete_Res)(nil),          // 17: typedb.protocol.CoreDatabase.Delete.Res
	(*Session_Open_Res)(nil),                 // 18: typedb.protocol.Session.Open.Res
	(*Session_Close_Res)(nil),                // 19: typedb.protocol.Session.Close.Res
	(*Session_Pulse_Res)(nil),                // 20: typedb.protocol.Session.Pulse.Res
	(*Transaction_Server)(nil),               // 21: typedb.protocol.Transaction.Server
}
var file_v2_protobuf_core_core_service_proto_depIdxs = []int32{
	0,  // 0: typedb.protocol.TypeDB.databases_contains:input_type -> typedb.protocol.CoreDatabaseManager.Contains.Req
	1,  // 1: typedb.protocol.TypeDB.databases_create:input_type -> typedb.protocol.CoreDatabaseManager.Create.Req
	2,  // 2: typedb.protocol.TypeDB.databases_all:input_type -> typedb.protocol.CoreDatabaseManager.All.Req
	3,  // 3: typedb.protocol.TypeDB.database_schema:input_type -> typedb.protocol.CoreDatabase.Schema.Req
	4,  // 4: typedb.protocol.TypeDB.database_type_schema:input_type -> typedb.protocol.CoreDatabase.TypeSchema.Req
	5,  // 5: typedb.protocol.TypeDB.database_rule_schema:input_type -> typedb.protocol.CoreDatabase.RuleSchema.Req
	6,  // 6: typedb.protocol.TypeDB.database_delete:input_type -> typedb.protocol.CoreDatabase.Delete.Req
	7,  // 7: typedb.protocol.TypeDB.session_open:input_type -> typedb.protocol.Session.Open.Req
	8,  // 8: typedb.protocol.TypeDB.session_close:input_type -> typedb.protocol.Session.Close.Req
	9,  // 9: typedb.protocol.TypeDB.session_pulse:input_type -> typedb.protocol.Session.Pulse.Req
	10, // 10: typedb.protocol.TypeDB.transaction:input_type -> typedb.protocol.Transaction.Client
	11, // 11: typedb.protocol.TypeDB.databases_contains:output_type -> typedb.protocol.CoreDatabaseManager.Contains.Res
	12, // 12: typedb.protocol.TypeDB.databases_create:output_type -> typedb.protocol.CoreDatabaseManager.Create.Res
	13, // 13: typedb.protocol.TypeDB.databases_all:output_type -> typedb.protocol.CoreDatabaseManager.All.Res
	14, // 14: typedb.protocol.TypeDB.database_schema:output_type -> typedb.protocol.CoreDatabase.Schema.Res
	15, // 15: typedb.protocol.TypeDB.database_type_schema:output_type -> typedb.protocol.CoreDatabase.TypeSchema.Res
	16, // 16: typedb.protocol.TypeDB.database_rule_schema:output_type -> typedb.protocol.CoreDatabase.RuleSchema.Res
	17, // 17: typedb.protocol.TypeDB.database_delete:output_type -> typedb.protocol.CoreDatabase.Delete.Res
	18, // 18: typedb.protocol.TypeDB.session_open:output_type -> typedb.protocol.Session.Open.Res
	19, // 19: typedb.protocol.TypeDB.session_close:output_type -> typedb.protocol.Session.Close.Res
	20, // 20: typedb.protocol.TypeDB.session_pulse:output_type -> typedb.protocol.Session.Pulse.Res
	21, // 21: typedb.protocol.TypeDB.transaction:output_type -> typedb.protocol.Transaction.Server
	11, // [11:22] is the sub-list for method output_type
	0,  // [0:11] is the sub-list for method input_type
	0,  // [0:0] is the sub-list for extension type_name
	0,  // [0:0] is the sub-list for extension extendee
	0,  // [0:0] is the sub-list for field type_name
}

func init() { file_v2_protobuf_core_core_service_proto_init() }
func file_v2_protobuf_core_core_service_proto_init() {
	if File_v2_protobuf_core_core_service_proto != nil {
		return
	}
	file_v2_protobuf_core_core_database_proto_init()
	file_v2_protobuf_common_session_proto_init()
	file_v2_protobuf_common_transaction_proto_init()
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: file_v2_protobuf_core_core_service_proto_rawDesc,
			NumEnums:      0,
			NumMessages:   0,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_v2_protobuf_core_core_service_proto_goTypes,
		DependencyIndexes: file_v2_protobuf_core_core_service_proto_depIdxs,
	}.Build()
	File_v2_protobuf_core_core_service_proto = out.File
	file_v2_protobuf_core_core_service_proto_rawDesc = nil
	file_v2_protobuf_core_core_service_proto_goTypes = nil
	file_v2_protobuf_core_core_service_proto_depIdxs = nil
}
